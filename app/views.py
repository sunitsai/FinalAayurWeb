...
def Addpost(request):
    if 'username' in request.session:
        ...
        newpost = Post.objects.create(cat_id=cid, Title=title, shortcontent=shortcontent,
                                      content=content, keyword=keyword, author=author, image=blogimg, authorimg=authorimg)
...
def InsertSeller(request):
    ...
    newseller = Seller.objects.create(
        Firstname=fname, Lastname=lname, Email=email, Contact=contact, Businessname=bname)
...
def Postcomment(request, pk):
    if request.method == "POST":
        ...
        newcomment = PostComment.objects.create(
            post_id=pid, Firstname=fname, Lastname=lname, Email=email, Contact=contact, comment=comment)
...
def Terms_Conditions(request):
    return render(request, "app/Term-condition.html")


def AboutOwnerPage(request):
    return render(request, "app/about-owner.html")
